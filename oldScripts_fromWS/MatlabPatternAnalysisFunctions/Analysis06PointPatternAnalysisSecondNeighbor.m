function Analysis06PointPatternAnalysisSecondNeighbor(path,filename,k,x,y,z,S,d123_all,d123_1)
...
%        dn(i)=dtemp(2);
    ...
    xlabel('distance to second nearest neighbor (in cell diameter)');
...
%         text(0.5,0.85,[num2str(d1min),'µm min dist.' ])
        title(['Type III (random permutations in I+II+III) CellDiameter=',num2str(CellDiameter,2),'µm'],'FontSize',10)
        saveas(gcf,[path,filename(1:end-4),'Case',num2str(k),'_Analysis06Fig1'], 'tiffn');
    clear dn
    clear G
